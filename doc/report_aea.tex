...
Table~\ref{tab:family_summary} compares mean gap to the area lower bound for
all ten heuristic methods across the three benchmark families.
NFDH and FFDH are shelf heuristics; BLF and Skyline each offer four sort-order
variants (DH/DW/DA/DP), with the best-of-four also reported.
BLF consistently dominates: its best-of-four achieves the lowest mean gap on
every family, while NFDH is the weakest method by a large margin.

\begin{table}[h]
\centering
\caption{Mean gap to area lower bound (\%) per benchmark family for all ten
         heuristic methods (60 training instances; best value per row in
         \textbf{bold}).}
...
\toprule
& &
  \multicolumn{2}{c}{Shelf} &
  \multicolumn{5}{c}{BLF} &
  Skyline \\
\cmidrule(lr){3-4}\cmidrule(lr){5-9}
Family & $N$ & NFDH & FFDH & DH & DW & DA & DP & best & best \\
\midrule
Bengtsson        & 10 & 33.32 & 12.70 & 13.77 &  7.35 &  7.95 &  9.42 & \textbf{7.02} & 25.30 \\
Berkey \& Wang   & 30 & 25.94 & 13.39 & 10.26 & 11.77 & 10.59 &  9.82 & \textbf{7.52} & 19.44 \\
Martello \& Vigo & 20 & 27.54 & 13.01 & 13.02 & 11.05 & 11.84 & 11.86 & \textbf{9.39} & 23.11 \\
\midrule
\textbf{Overall} & \textbf{60} & 27.70 & 13.15 & 11.77 & 10.79 & 10.57 & 10.43 & \textbf{8.06} & 21.64 \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:selection_dist} compares these true best-performer counts
against the algorithms actually chosen by RF and MLP.
...
\begin{axis}[
    ...
    width      = 13cm,
    height     = 6.5cm,
    symbolic x coords = {BLF-DW, BLF-DH, BLF-DA, BLF-DP, FFDH},
    xtick      = data,
    xlabel     = {Algorithm},
    ylabel     = {Times selected (out of 100)},
    ymin = 0, ymax = 60,
    ymajorgrids = true,
...
% Oracle (true best performer)
\addplot[fill=teal!65!black, draw=teal!30!black]
    coordinates {(BLF-DW,33)(BLF-DH,19)(BLF-DA,18)(BLF-DP,16)(FFDH,14)};
\addlegendentry{Oracle}
% Random Forest predictions
\addplot[fill={rgb,1:red,0.31;green,0.56;blue,0.65}, draw=blue!50]
    coordinates {(BLF-DW,49)(BLF-DH,20)(BLF-DA,12)(BLF-DP,7)(FFDH,12)};
\addlegendentry{RF}
% MLP predictions
\addplot[fill=orange!70, draw=orange!90!black]
    coordinates {(BLF-DW,32)(BLF-DH,16)(BLF-DA,27)(BLF-DP,16)(FFDH,9)};
\addlegendentry{MLP}
\end{axis}
\end{tikzpicture}
\caption{Algorithm selection frequency on 100 test instances: oracle (true
         best performer) vs.\ RF vs.\ MLP predictions.
         RF over-selects BLF-DW (49 vs.\ 33 actual) and severely under-selects
         BLF-DP (7 vs.\ 16), biasing towards width-sorted BLF.
         MLP over-selects BLF-DA (27 vs.\ 18 actual) and under-selects FFDH
         (9 vs.\ 14), reflecting different learned feature interactions.
         No model ever selects any Skyline variant.}
\label{fig:selection_dist}
\end{figure}
